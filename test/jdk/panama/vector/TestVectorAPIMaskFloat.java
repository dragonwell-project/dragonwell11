 ...
 *   @run testng/othervm --add-opens jdk.panama.vector/jdk.panama.vector=ALL-UNNAMED -XX:+UseVectorAPI TestVectorAPIMaskFloat
 *
 */

public class  TestVectorAPIMaskFloat
{
    public static int size = 1024 * 16;
    static Random random = new Random();
    static final FloatVector.FloatSpecies Species128 = FloatVector.species(Shape.S_128_BIT);
    static final FloatVector.FloatSpecies Species256 = FloatVector.species(Shape.S_256_BIT);
    static final FloatVector.FloatSpecies Species512 = FloatVector.species(Shape.S_512_BIT);
    static final FloatVector.FloatSpecies Species = FloatVector.preferredSpecies();
    static boolean[] AisNull = new boolean[size];
    static float[] result = new float[size];
    static float[] resultv = new float[size];
    static float[] input = new float[size];
    ...
    static void vecTest128() {
        FloatVector v0;
        FloatVector.FloatSpecies Species = Species128;
        int i = 0;
        Mask mask0;
    ...
    static void vecTest256() {
        FloatVector v0;
        FloatVector.FloatSpecies Species = Species256;
        int i = 0;
        Mask mask0;
        for (i = 0; i + (Species.length()) <= size; i += Species.length()) {
            mask0 = Species.maskFromArray(AisNull, i);
    ...
    static void vecTest512() {
        FloatVector v0;
        FloatVector.FloatSpecies Species = Species512;
        int i = 0;
        Mask mask0;
        for (i = 0; i + (Species.length()) <= size; i += Species.length()) {
            mask0 = Species.maskFromArray(AisNull, i);
