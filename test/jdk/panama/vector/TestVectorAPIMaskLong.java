 ...
 *   @run testng/othervm --add-opens jdk.panama.vector/jdk.panama.vector=ALL-UNNAMED -XX:+UseVectorAPI TestVectorAPIMaskLong
 *
 */

public class  TestVectorAPIMaskLong
{
    public static int size = 1024 * 16;
    static Random random = new Random();
    static final LongVector.LongSpecies Species128 = LongVector.species(Shape.S_128_BIT);
    static final LongVector.LongSpecies Species256 = LongVector.species(Shape.S_256_BIT);
    static final LongVector.LongSpecies Species512 = LongVector.species(Shape.S_512_BIT);
    static final LongVector.LongSpecies Species = LongVector.preferredSpecies();
    static boolean[] AisNull = new boolean[size];
    static long[] result = new long[size];
    static long[] resultv = new long[size];
    static long[] input = new long[size];
    ...
    static void vecTest128() {
        LongVector v0;
        LongVector.LongSpecies Species = Species128;
        int i = 0;
        Mask mask0;
    ...
    static void vecTest256() {
        LongVector v0;
        LongVector.LongSpecies Species = Species256;
        int i = 0;
        Mask mask0;
        for (i = 0; i + (Species.length()) <= size; i += Species.length()) {
            mask0 = Species.maskFromArray(AisNull, i);
    ...
    static void vecTest512() {
        LongVector v0;
        LongVector.LongSpecies Species = Species512;
        int i = 0;
        Mask mask0;
        for (i = 0; i + (Species.length()) <= size; i += Species.length()) {
            mask0 = Species.maskFromArray(AisNull, i);
