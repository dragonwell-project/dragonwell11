 ...
 *   @run testng/othervm --add-opens jdk.panama.vector/jdk.panama.vector=ALL-UNNAMED -XX:+UseVectorAPI TestVectorAPIMaskByte
 *
 */

public class  TestVectorAPIMaskByte
{
    public static int size = 1024 * 16;
    static Random random = new Random();
    static final ByteVector.ByteSpecies Species128 = ByteVector.species(Shape.S_128_BIT);
    static final ByteVector.ByteSpecies Species256 = ByteVector.species(Shape.S_256_BIT);
    static final ByteVector.ByteSpecies Species512 = ByteVector.species(Shape.S_512_BIT);
    static final ByteVector.ByteSpecies Species = ByteVector.preferredSpecies();
    static boolean[] AisNull = new boolean[size];
    static byte[] result = new byte[size];
    static byte[] resultv = new byte[size];
    ...
    public static void main(String[] args) throws  NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException, InstantiationException {
        ...
        int i = 0;
        for (i = 0; i < size; i++) {
            input[i] = (byte)random.nextInt(50);
            result[i] = 0;
            resultv[i] = 0;
    ...
    static void vecTest128() {
        ByteVector v0;
        ByteVector.ByteSpecies Species = Species128;
        int i = 0;
        Mask mask0;
    ...
    static void vecTest256() {
        ByteVector v0;
        ByteVector.ByteSpecies Species = Species256;
        int i = 0;
        Mask mask0;
        for (i = 0; i + (Species.length()) <= size; i += Species.length()) {
            mask0 = Species.maskFromArray(AisNull, i);
    ...
    static void vecTest512() {
        ByteVector v0;
        ByteVector.ByteSpecies Species = Species512;
        int i = 0;
        Mask mask0;
        for (i = 0; i + (Species.length()) <= size; i += Species.length()) {
            mask0 = Species.maskFromArray(AisNull, i);
