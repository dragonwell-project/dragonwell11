...
LogRun true "${TESTS}\n"
LogRun false "${JAVA} -cp \"${VECTORTESTS_HOME_CP}${SEPARATOR}${TESTNG_RUN_JAR}${SEPARATOR}${JCOMMANDER_JAR}\" ${JAVA_FLAGS} --add-modules jdk.panama.vector --add-opens jdk.panama.vector/jdk.panama.vector=ALL-UNNAMED org.testng.TestNG -testclass $TESTS"

# Actual TestNG run.
time ${JAVA} -cp "${VECTORTESTS_HOME_CP}${SEPARATOR}${TESTNG_RUN_JAR}${SEPARATOR}${JCOMMANDER_JAR}" \
  ${JAVA_FLAGS} \
  --add-modules jdk.panama.vector \
  --add-opens jdk.panama.vector/jdk.panama.vector=ALL-UNNAMED \
  -Djdk.panama.vector.test.loop-iterations=${TEST_ITER_COUNT} \
