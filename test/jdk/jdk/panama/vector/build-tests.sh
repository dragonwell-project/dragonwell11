...
do
  ...
  do
    ...
    Log false "\n${JAVAC} -cp \"${VECTORTESTS_HOME_CP}$SEPARATOR${TESTNG_JAR}\" --add-modules=jdk.panama.vector $vectorteststype.java 2>&1"
    compilation=$(${JAVAC} -cp "${VECTORTESTS_HOME_CP}$SEPARATOR${TESTNG_JAR}" \
                    --add-modules=jdk.panama.vector $vectorteststype.java 2>&1)
