...
const char* QuickStart::_opt_name[Count] = {
  "appcds",
  "eagerappcds",
};

bool QuickStart::_opt_enabled[Count] = {
  true,  // appcds
  true,  // eagerappcds
};

bool QuickStart::_opt_passed[Count] = {};  // all false
...
bool QuickStart::parse_command_line_arguments(const char* options) {
  ...
  GrowableArray<const char *> *options_array = new (ResourceObj::C_HEAP, mtArguments) GrowableArray<const char *>(5, true);
...
void QuickStart::print_command_line_help(outputStream* out) {
  ...
  out->print_cr("          -Xquickstart:verbose,+eagerappcds");
  out->cr();
}
...
void QuickStart::check_features(const char* &str) {
  ...
  for (int begin = 0, end = 0; ; ) {
    ...
    if (str[end] == ',' || (exit = (str[end] == '\n'))) {
      // handle feature
      for (int i = 0; i < QuickStart::Count; i++) {
        int len = (int)MIN2(::strlen(&str[begin]), ::strlen(_opt_name[i]));
...
bool QuickStart::load_and_validate(JavaVMInitArgs* options_args) {
  ...
  while (fgets(line, sizeof(line), _metadata_file) != NULL) {
    ...
    if (!feature_checked && match_option(line, _identifier_name[Features], &tail)) {
      ...
      // ignore \n
      int size = (int)strlen(tail) - 1;
      const char *image_ident = QuickStart::image_id();
      int ident_size = image_ident != NULL ? (int)strlen(image_ident) : 0;
      if (size != ident_size) {
      ...
      option_checked = true;
      // We delegate argument checking logic to CDS itself.
