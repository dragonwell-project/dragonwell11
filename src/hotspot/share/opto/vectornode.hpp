...
public:
  AddReductionVINode(Node * ctrl, Node* in1, Node* in2) : ReductionNode(ctrl, in1, in2) {
    if (UseVectorAPI && in1->bottom_type()->basic_type() == T_INT) {
...
public:
  ...
  virtual int Opcode() const;
  virtual const Type* bottom_type() const { return TypeLong::LONG; }
  virtual uint ideal_reg() const { return UseVectorAPI ? Op_RegL : Op_RegI; }
};
...
class StoreVectorNode : public StoreNode {
 public:
  StoreVectorNode(Node* c, Node* mem, Node* adr, const TypePtr* at, Node* val)
    ...
    assert(val->is_Vector() || val->is_LoadVector() || UseVectorAPI, "sanity");
...
class ExtractCNode : public ExtractNode {
 public:
  ...
  virtual int Opcode() const;
  virtual const Type *bottom_type() const { return UseVectorAPI ? TypeInt::CHAR : TypeInt::INT; }
  virtual uint ideal_reg() const { return Op_RegI; }
};
...
class ExtractSNode : public ExtractNode {
 public:
  ...
  virtual int Opcode() const;
  virtual const Type *bottom_type() const { return UseVectorAPI ? TypeInt::SHORT : TypeInt::INT; }
  virtual uint ideal_reg() const { return Op_RegI; }
};
