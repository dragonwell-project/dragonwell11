...
public:
  ...
  virtual void work() {
    for (;;) {
      ...
      const uintptr_t offset = Atomic::add(size, &_start) - size;
      if (offset >= _end) {
        // Done
        break;
      }
...
size_t ZPageAllocator::increase_capacity(size_t size) {
  ...
  if (increased > 0) {
    ...
    Atomic::add(increased, &_capacity);
...
void ZPageAllocator::decrease_capacity(size_t size, bool set_max_capacity) {
  ...
  if (set_max_capacity) {
    ...
    // Update atomically since we have concurrent readers
    Atomic::store(_capacity, &_current_max_capacity);
  }
}
...
void ZPageAllocator::increase_used(size_t size, bool relocation) {
  ...
  // Update atomically since we have concurrent readers
  const size_t used = Atomic::add(size, &_used);
  if (used > _used_high) {
    _used_high = used;
  }
}
...
void ZPageAllocator::decrease_used(size_t size, bool reclaimed) {
  ...
  // Update atomically since we have concurrent readers
  const size_t used = Atomic::sub(size, &_used);
  if (used < _used_low) {
    _used_low = used;
  }
}
...
size_t ZPageAllocator::uncommit(uint64_t* timeout) {
...

  {
    ...
    // are uncommitting.
    const size_t retain = MIN2(MAX2(_used + _max_reserve, _min_capacity), _capacity);
