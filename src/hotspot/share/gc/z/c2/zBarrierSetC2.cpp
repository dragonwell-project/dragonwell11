...
void ZBarrierSetC2::expand_loadbarrier_node(PhaseMacroExpand* phase, LoadBarrierNode* barrier) const {
  ...
  phase->C->print_method(PHASE_BARRIER_EXPANSION, 4);
}
...
bool ZBarrierSetC2::matcher_find_shared_post_visit(Matcher* matcher, Node* n, uint opcode) {
  switch(opcode) {
    case Op_ZCompareAndExchangeP:
    case Op_ZCompareAndSwapP:
    case Op_ZWeakCompareAndSwapP: {
...
// verify_gc_barriers is not backported at this point
...
static void call_catch_cleanup_one(PhaseIdealLoop* phase, LoadNode* load, Node* ctrl) {
  ...
  phase->C->print_method(PHASE_CALL_CATCH_CLEANUP, 4);
...
void ZBarrierSetC2::barrier_insertion_phase(Compile* C, PhaseIterGVN& igvn) {
  PhaseIdealLoop ideal_loop(igvn, LoopOptsZBarrierInsertion);
  if (C->failing())  return;
}
...
static void insert_barrier_before_unsafe(PhaseIdealLoop* phase, LoadStoreNode* old_node) {
  ...
  if (zclone != NULL) {
    ...
    C->print_method(PHASE_ADD_UNSAFE_BARRIER, 4);
  }
}
...
void ZBarrierSetC2::insert_one_loadbarrier_inner(PhaseIdealLoop* phase, LoadNode* load, Node* ctrl, VectorSet visited2) const {
  ...
  C->print_method(PHASE_INSERT_BARRIER, 3);
}
...
bool ZBarrierSetC2::needs_anti_dependence_check(const Node* node) {
