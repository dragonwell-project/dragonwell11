...
class ZNMethodTableUnloadCacheClosure : public ZNMethodTableEntryClosure {
private:
  bool          _unloading_occurred;

public:
  ZNMethodTableUnloadCacheClosure(bool unloading_occurred) :
  ...
  virtual void do_nmethod_entry(ZNMethodTableEntry entry) {
    ...
    {
      ResourceMark rm;
      nm->unload_nmethod_caches_ZGC(_unloading_occurred);
    }
  }
};

class ZNMethodTableUnloadCacheTask : public ZTask {
private:
  ZNMethodTableUnloadCacheClosure _cl;

public:
  ZNMethodTableUnloadCacheTask(bool unloading_occurred) :
      ZTask("ZNMethodTableUnloadCacheTask"),
      _cl(unloading_occurred) {
    ZNMethodTable::nmethod_entries_do_begin();
  }

  ~ZNMethodTableUnloadCacheTask() {
    ZNMethodTable::nmethod_entries_do_end();
  }
...
void ZNMethodTable::unlink_and_purge(ZWorkers* workers, bool unloading_occurred) {
  {
    ZNMethodTableUnlinkTask task;
    workers->run_parallel(&task);
  }
  {
    ZNMethodTableUnloadCacheTask task(unloading_occurred);
    workers->run_parallel(&task);
  }
}
