...
void ContiguousSpace::allocate_temporary_filler(int factor) {
  ...
  const size_t array_header_size = UseCompactObjectHeaders ? ((arrayOopDesc::base_offset_in_bytes(T_INT) + BytesPerWord) / BytesPerWord) : typeArrayOopDesc::header_size(T_INT);
