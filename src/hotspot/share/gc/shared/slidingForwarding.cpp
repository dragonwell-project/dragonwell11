...
HeapWord* SlidingForwarding::_heap_start = NULL;
...
HeapWord** SlidingForwarding::_biased_bases[SlidingForwarding::NUM_TARGET_REGIONS] = { NULL, NULL };
...
void SlidingForwarding::initialize(MemRegion heap, size_t region_size_words) {
  ...
  if (UseAltGCForwarding) {
    ...
    assert(_bases_table == NULL, "should not be initialized yet");
    assert(_fallback_table == NULL, "should not be initialized yet");
  }
#endif
}
...
HeapWord* SlidingForwarding::fallback_forwardee(HeapWord* from) {
  assert(_fallback_table != NULL, "fallback table must be present");
...
void SlidingForwarding::FallbackTable::forward_to(HeapWord* from, HeapWord* to) {
  ...
  for (FallbackTableEntry* entry = head; entry != NULL; entry = entry->_next) {
