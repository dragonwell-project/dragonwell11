...
uintptr_t SlidingForwarding::encode_forwarding(HeapWord* from, HeapWord* to) {
  assert(NUM_TARGET_REGIONS == 2, "Only implemented for this amount");
    ...
    *base = to_region_base;
    ...
    if (*base == to_region_base) {
      ...
      *base = to_region_base;
      ...
      return (1 << FALLBACK_SHIFT) | markOopDesc::marked_value;
    }
    alternate = 1;
  }
...
HeapWord* SlidingForwarding::decode_forwarding(HeapWord* from, uintptr_t encoded) {
  assert((encoded & markOopDesc::lock_mask_in_place) == markOopDesc::marked_value, "must be marked as forwarded");
...
inline void SlidingForwarding::forward_to_impl(oop from, oop to) {
  assert(_bases_table != NULL, "call begin() before forwarding");

  markOop from_header = from->mark();
  ...
  markOop new_header = markOop((from_header->value() & ~MARK_LOWER_HALF_MASK) | encoded);
  assert((HeapWord*)new_header->clear_lock_bits()!= from_hw, "sanity");
...
#ifdef _LP64
  if (ALT_FWD) {
    assert(_bases_table != NULL, "expect sliding forwarding initialized");
...
inline oop SlidingForwarding::forwardee_impl(oop from) {
  assert(_bases_table != NULL, "call begin() before asking for forwarding");

  markOop header = from->mark();
  HeapWord* from_hw = cast_from_oop<HeapWord*>(from);
