...
nmethod* SharedRuntime::generate_native_wrapper(MacroAssembler* masm,
  ...
  if (method->is_synchronized()) {
    ...
    if (UseAltFastLocking) {
      ...
      __ tbnz(old_hdr, exact_log2(markOopDesc::monitor_value), slow_path_unlock);
...
void create_switchTo_contents(MacroAssembler *masm, int start, OopMapSet* oop_maps, int &stack_slots, int total_in_args,
...

  {
    ...
    __ strw(temp, Address(target_coroutine, Coroutine::state_offset()));
    {
      ...
      __ ldr(temp2, Address(target_stack, CoroutineStack::stack_overflow_limit_offset()));
      __ str(temp2, Address(thread, JavaThread::stack_overflow_limit_offset()));
