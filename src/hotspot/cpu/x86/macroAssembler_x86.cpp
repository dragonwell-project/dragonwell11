...
void MacroAssembler::lookup_secondary_supers_table_slow_path(Register r_super_klass,
  ...
  { // Degenerate case: more than 64 secondary supers.
    ...
    repne_scanq(r_array_base, r_super_klass, r_array_index, r_array_length,
                L_success,
                (&L_fallthrough != L_failure ? L_failure : NULL));
...
void MacroAssembler::verify_secondary_supers_table(Register r_sub_klass,
  ...
  { // To avoid calling convention issues, build a record on the stack
    ...
    push(r_super_klass);
    movptr(c_rarg1, rsp);
    movptr(c_rarg0, (intptr_t) "mismatch");
...
void MacroAssembler::encode_klass_not_null(Register r, Register tmp) {
  ...
  if (Universe::narrow_klass_base() != NULL) {
    mov64(tmp, (int64_t)Universe::narrow_klass_base());
    subq(r, tmp);
  }
...
void MacroAssembler::fast_lock_impl(Register obj, Register hdr, Register thread, Register tmp, Label& slow) {
  ...
  andptr(hdr, ~(int32_t)markOopDesc::lock_mask_in_place);
  movptr(tmp, hdr);
...
void MacroAssembler::fast_unlock_impl(Register obj, Register hdr, Register tmp, Label& slow) {
  ...
  orptr(tmp, markOopDesc::unlocked_value);
  lock();
  cmpxchgptr(tmp, Address(obj, oopDesc::mark_offset_in_bytes()));
  jcc(Assembler::notEqual, slow);
