...
public class WispEngine extends AbstractExecutorService {
    ...
    private static Map<Long, WispCounter> managedEngineCounters;
    ...
    private static void initializeWispClass() {
        ...
        managedEngineCounters = new ConcurrentSkipListMap<>();
    ...
    private static void setWispEngineAccess() {
        SharedSecrets.setWispEngineAccess(new WispEngineAccess() {
            ...
            public int epollWait(int epfd, long pollArray, int arraySize, long timeout,
                ...
                return WispEventPump.Pool.INSTANCE.eventWaitForWisp(epfd, pollArray, arraySize, timeout, status, INTERRUPTED);
            }
    ...
    static void registerPerfCounter(WispCarrier carrier) {
        runInCritical(() -> {
            ...
            managedEngineCounters.put(carrier.getId(), carrier.counter);
            return null;
        });
    }
    ...
    static void deRegisterPerfCounter(WispCarrier carrier) {
        runInCritical(() -> {
            ...
            managedEngineCounters.remove(carrier.getId());
            carrier.counter.cleanup();
            return null;
        });
    }
