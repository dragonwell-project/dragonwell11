...
public class TenantContainer {
     ...
     */
    private List<WeakReference<Thread>> spawnedThreads = Collections.synchronizedList(new ArrayList<WeakReference<Thread>>());

    /*
     * Used to contain service threads, including finalizer threads, shutdown hook threads.
     */
    private Map<Thread, Void> serviceThreads = Collections.synchronizedMap(new WeakHashMap<Thread, Void>());
    ...
    private static void initializeTenantContainerClass() {
        ...
        tenantContainerMap = Collections.synchronizedMap(new HashMap<Long, TenantContainer>());
